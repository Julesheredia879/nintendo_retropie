...
vmin = {"discharging": 8,
        "charging"   : 4.25 }
icons = { "discharging": [ "alert_red", "alert", "20", "30", "40", "50", "60",
                           "70", "80", "90", "full", "full" ],
...
def iconos_en_pantalla():
  ...
  while True:
    ...
    bt_state = bluetooth()
    env = environment()
    my_logger.info("%s,Batery: %.2f, Array:  %s,icon: %s,wifi: %s,bt: %s, throttle: %#0x" % (
    datetime.now(),
    value_v,
...
if __name__ == "__main__":
    try:
        ...
        t1.start()
        t2= Thread( target = iconos_en_pantalla)
        t2.start()
    except:
        t1.stop()
        t2.stop()
